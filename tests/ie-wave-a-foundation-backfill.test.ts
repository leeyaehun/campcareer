...
const migrationSql = readFileSync(
  "supabase/migrations/20260912191500_ie_wave_a_career_foundation_backfill.sql",
  "utf8",
)
...
test("committed Ireland migration preserves the generated backfill safety contract", () => {
  assert.match(migrationSql, /begin;/)
  assert.match(migrationSql, /commit;\s*$/)
  assert.equal((migrationSql.match(/insert into public\.career_foundation_profiles/g) ?? []).length, 8)
  assert.equal((migrationSql.match(/'EUR',false,/g) ?? []).length, 8)
  assert.doesNotMatch(migrationSql, /'EUR',true,/)
  assert.match(migrationSql, /expected 8 foundation profiles/)
  assert.match(migrationSql, /expected 6 evidence-complete score rows/)
  assert.match(migrationSql, /unexpectedly produced % publish-ready rows/)
})

test("committed Ireland migration keeps six complete and two incomplete profiles", () => {
  for (const careerId of [
    "software-developer",
    "cybersecurity-analyst",
    "data-engineer",
    ...
    assert.match(migrationSql, new RegExp(`IE:${careerId}:2026-09-12:wave-a-v1`))
  }

  for (const careerId of ["accountant", "architect"]) {
    assert.match(migrationSql, new RegExp(`IE:${careerId}.*unavailable`, "s"))
  }
})
