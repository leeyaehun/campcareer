    ...
    <main className="min-h-[calc(100vh-4rem)] bg-campcareer-canvas px-4 py-8 sm:px-8 sm:py-12">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mt-6 max-w-3xl">
          ...
          <p className="mt-3 text-sm leading-6 text-campcareer-ink-secondary">Review verified study, graduate-work and employment-permit pathways. Eligibility and requirements depend on your circumstances; confirm the current position with the issuing authority.</p>
        </div>
        <div className="mt-7 grid gap-3 sm:grid-cols-2">
          {visas.map((visa) => (
            <Link key={visa.name} href={irelandVisaCanonicalPath(visa.name)} className="group block rounded-cc-surface border border-campcareer-border bg-campcareer-surface p-5 shadow-cc-surface transition hover:-translate-y-0.5 hover:border-brand/30 hover:shadow-md focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-ring/20">
              <p className="text-xs font-semibold uppercase tracking-[0.08em] text-brand">{visa.kind}</p>
              <h2 className="mt-2 flex items-center gap-1.5 text-lg font-semibold text-campcareer-ink group-hover:text-brand">{visa.name}<ArrowRight className="size-4 transition-transform group-hover:translate-x-0.5" /></h2>
        ...
        <p className="mt-6 flex gap-2 rounded-cc-surface border border-amber-200 bg-amber-50 p-4 text-xs leading-5 text-amber-950"><ShieldAlert className="mt-0.5 size-4 shrink-0" />This is general country context, not immigration or legal advice. Rules, fees and processing times can change.</p>
      </div>
    </main>
  )
}
    ...
    <main className="min-h-[calc(100vh-4rem)] bg-campcareer-canvas px-4 py-8 sm:px-8 sm:py-12">
      <div className="mx-auto max-w-4xl">
        ...
        <article className="mt-6 rounded-cc-large border border-campcareer-border bg-campcareer-surface p-5 shadow-cc-surface sm:p-8">
          <p className="text-xs font-semibold uppercase tracking-[0.08em] text-brand">{visa.kind} pathway · Ireland</p>
          <h1 className="mt-2 text-3xl font-bold tracking-[-0.04em] text-campcareer-ink sm:text-4xl">{visa.name}</h1>
